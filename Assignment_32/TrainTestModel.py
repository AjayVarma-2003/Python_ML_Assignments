...
def HardVotingPredictor(X_train, X_test, Y_train, Y_test):
    # building models
...
def SoftVotingPredictor(X_train, X_test, Y_train, Y_test):
    # building models
    model1 = LogisticRegression(max_iter = 1000)
    model2 = DecisionTreeClassifier(max_depth = 8)
    model3 = KNeighborsClassifier(n_neighbors = 5)
...
def NewsClassifier(dataset1, dataset2):
    ...
    accuracy = HardVotingPredictor(X_train, X_test, Y_train, Y_test)
    print("Accuracy of model using hard voting is : ", accuracy)

    accuracy = SoftVotingPredictor(X_train, X_test, Y_train, Y_test)
    print("Accuracy of model using soft voting is : ", accuracy)
